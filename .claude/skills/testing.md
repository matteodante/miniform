...
# Run real-process HTTP, SQLite, webhook, SMTP, and resource stress coverage
make test-stress
...
The stress suite uses a loopback SMTP capture that accepts only `.invalid` recipients and injects transient failures. It never requires or contacts an external SMTP service.
