...
HTML form -> public HTTP route -> validation -> upload staging -> SQLite submission
                                                                   |
                                                                   +-> webhook queue
                                                                   +-> SMTP queue (one event per notification)
...
Webhook and email dispatchers run in process under the application context. Database rows are the durable queue and delivery history. `next_attempt_at` is both the queue eligibility field and, while delivering, an expiring lease. A worker claims one row transactionally, reloads its current delivery configuration, performs network I/O outside the transaction, and updates only the lease it still owns. Expired work is reclaimable; terminal events set `next_attempt_at` to `NULL`.

Webhook requests carry a stable idempotency key. SMTP and webhook calls honor cancellation, use bounded attempts, compact stored errors, and never keep a database transaction open during network I/O. Each enabled email notification creates a separate event linked to its configuration, so delivery state and retries remain independent. The forms domain owns one email renderer shared by the authenticated preview route and the background SMTP worker; preview reads a submission but never creates an event or opens a network connection. Email delivery validates every envelope recipient and `Reply-To` before issuing `MAIL FROM`, sends one `RCPT TO` per configured static recipient, and renders either quoted-printable text or escaped `multipart/alternative` HTML with a text fallback. A manual retry resets the durable event; delivery still requires a running server process.
