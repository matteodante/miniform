...
The legacy form flags configure the first email notification. They require `--email-enabled`, `--mailer-profile-id`, and `--email-recipient`. The recipient flag accepts an RFC 5322 comma-separated list; use `--email-format text` for plain text or `--email-format html` for HTML with an automatic text fallback:
...
Omitted update flags preserve the current recipients and format. Duplicate addresses are removed case-insensitively. Use the `email` commands below for multiple notifications, field-derived addresses, Reply-To, subjects, and templates. Webhook forwarding requires `--webhook-enabled` and `--webhook-url`.
...
List and inspect the independent notifications owned by a form:
...
`email list` omits message bodies to stay compact. `email get` returns the effective subject, text template, and HTML template.

Create an internal notification whose Reply-To comes from the submitted `email` field:
...
miniform --json email create \
  ...
  --recipient 'bookings@example.com, archive@example.com' \
  --reply-to-source field \
  --reply-to email \
  --subject-template 'New request · {{.Fields.name}}' \
  --format html \
  --text-template-file ./internal.txt \
  --html-template-file ./internal.html
```

Create a separate confirmation sent to the submitter:
...
miniform --json email create \
  ...
  --subject-template 'We received your request' \
  --format html \
  --text-template-file ./confirmation.txt \
  --html-template-file ./confirmation.html
```

Updates preserve omitted values:

```bash
miniform --json email update --id 2 --enabled=false
miniform --json email update --id 2 --reply-to-source field --reply-to email
```

`recipient-source` accepts `static` or `field`. `reply-to-source` accepts `none`, `static`, or `field`. Field mode stores a submitted field name, not a template expression. Templates use Go template syntax; see [Email notifications](email-notifications.md) for the available data, escaping, and delivery semantics.

Delete one notification without deleting its historical delivery events:
