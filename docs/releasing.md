...
3. Create a signed annotated tag from the exact `main` commit, replacing `vX.Y.Z` with the release version:

   ```bash
   git tag --sign vX.Y.Z -m "Miniform vX.Y.Z"
...
The published image is anonymously pullable. Verify its manifest and supported architectures with:

```bash
docker buildx imagetools inspect ghcr.io/matteodante/miniform:vX.Y.Z
```
