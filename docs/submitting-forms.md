...
Relative redirect paths resolve against the website that hosts the form. Absolute redirect URLs must use an allowed hostname. If no redirect field is present, Miniform returns JSON.

Add `enctype="multipart/form-data"` only when the form contains file inputs:
...
- The default scalar-field limit is `200` and is configurable with `MINIFORM_MAX_INPUT_FIELDS`.
- Files are limited to 10 MB each, 5 per field, and 10 total. Allowed extensions are `.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`, `.svg`, `.pdf`, `.doc`, `.docx`, `.xls`, `.xlsx`, `.ppt`, `.pptx`, `.txt`, `.csv`, `.rtf`, `.odt`, `.ods`, and `.odp`.
