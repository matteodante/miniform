...
type formView struct {
	...
	EmailDeliveries  []emailDeliveryView  `json:"email_deliveries,omitempty"`
...
type emailDeliveryView struct {
	...
	ReplyToSource   string `json:"reply_to_source"`
...
func updateParamsFromForm(form *forms.Form) forms.UpdateParams {
	...
	if delivery := forms.PrimaryEmailDelivery(form); delivery != nil {
		params.EmailName = delivery.Name
		params.EmailEnabled = delivery.Enabled
		params.MailerProfileID = delivery.MailerProfileID
		params.EmailRecipientType = delivery.RecipientSource
		params.EmailRecipient = delivery.Recipient
		params.EmailReplyToType = delivery.ReplyToSource
		params.EmailReplyTo = delivery.ReplyTo
		params.EmailSubject = delivery.SubjectTemplate
		params.EmailFormat = delivery.Format
		params.EmailText = delivery.TextTemplate
		params.EmailHTML = delivery.HTMLTemplate
	}
...
func newFormView(form *forms.Form, showSecrets bool) formView {
	...
	for i := range form.EmailDeliveries {
		deliveryView := newEmailDeliveryView(&form.EmailDeliveries[i], false)
		view.EmailDeliveries = append(view.EmailDeliveries, deliveryView)
	}
	if len(view.EmailDeliveries) > 0 {
		primary := view.EmailDeliveries[0]
		view.EmailDelivery = &primary
	}
	if form.WebhookDelivery != nil {
...
func newEmailDeliveryView(delivery *forms.EmailDelivery, includeTemplates bool) emailDeliveryView {
	view := emailDeliveryView{
		ID: delivery.ID, FormID: delivery.FormID, Name: delivery.Name,
		Enabled: delivery.Enabled, MailerProfileID: delivery.MailerProfileID,
		RecipientSource: delivery.RecipientSource, Recipient: forms.DisplayEmailAddresses(delivery.Recipient),
		ReplyToSource: delivery.ReplyToSource, ReplyTo: forms.DisplayEmailAddresses(delivery.ReplyTo),
		SubjectTemplate: forms.EffectiveEmailSubject(delivery), Format: normalizedEmailFormat(delivery.Format),
	}
	if includeTemplates {
		view.TextTemplate = forms.EffectiveEmailText(delivery)
		view.HTMLTemplate = forms.EffectiveEmailHTML(delivery)
	}
	return view
}
