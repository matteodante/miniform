...
func TestRunner(t *testing.T) {
	...
	t.Run("configures multiple email recipients and message format", func(t *testing.T) {
		...
		require.NotNil(t, delivery)
		assert.Equal(t, forms.EmailFormatText, delivery.Format)
	...
	t.Run("manages independent email notifications with dynamic addresses and templates", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(textPath, []byte("Hello {{.Fields.name}}"), 0o600))
		require.NoError(t, os.WriteFile(htmlPath, []byte("<p>Hello {{.Fields.name}}</p>"), 0o600))

		runner, stdout, _ := newTestRunner(t, db, "")
		exitCode := runner.Run([]string{
			"email", "create", "--json", "--form-id", uintString(form.ID), "--name", "Customer confirmation", "--enabled",
			"--mailer-profile-id", uintString(profile.ID), "--recipient-source", "field", "--recipient", "email",
			"--reply-to-source", "static", "--reply-to", "support@example.com",
			"--subject-template", "Thanks {{.Fields.name}}", "--format", "html",
			"--text-template-file", textPath, "--html-template-file", htmlPath,
		})
		assert.Equal(t, ExitSuccess, exitCode)
		assert.Contains(t, stdout.String(), `"recipient_source":"field"`)
		assert.Contains(t, stdout.String(), `"reply_to_source":"static"`)
		assert.Contains(t, stdout.String(), `"text_template":"Hello {{.Fields.name}}"`)

		deliveries, err := forms.ListEmailDeliveries(db, form.ID)
		require.NoError(t, err)
		...
		runner, _, _ = newTestRunner(t, db, "")
		exitCode = runner.Run([]string{
			"email", "update", "--id", uintString(created.ID), "--enabled=false",
			"--reply-to-source", "field", "--reply-to", "email",
		})
		assert.Equal(t, ExitSuccess, exitCode)
		updated, err := forms.GetEmailDeliveryByID(db, created.ID)
		require.NoError(t, err)
		assert.False(t, updated.Enabled)
		assert.Equal(t, forms.EmailReplyToField, updated.ReplyToSource)
		assert.Equal(t, "email", updated.ReplyTo)

		runner, stdout, _ = newTestRunner(t, db, "")
		exitCode = runner.Run([]string{"email", "list", "--json", "--form-id", uintString(form.ID)})
		assert.Equal(t, ExitSuccess, exitCode)
		assert.Contains(t, stdout.String(), "Customer confirmation")
		assert.NotContains(t, stdout.String(), `"text_template"`)

		runner, _, _ = newTestRunner(t, db, "")
		exitCode = runner.Run([]string{"email", "delete", "--id", uintString(created.ID), "--yes"})
		assert.Equal(t, ExitSuccess, exitCode)
		deliveries, err = forms.ListEmailDeliveries(db, form.ID)
		require.NoError(t, err)
