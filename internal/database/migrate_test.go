...
func TestMigrate(t *testing.T) {
	...
	t.Run("allows forms without email notifications and restores missing webhook records", func(t *testing.T) {
		db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
		require.NoError(t, err)
		require.NoError(t, Migrate(db))
	...
	t.Run("removes the legacy one-email-per-form uniqueness without losing configuration", func(t *testing.T) {
		db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
		require.NoError(t, err)
		require.NoError(t, db.AutoMigrate(&legacyForm{}, &partiallySimplifiedEmailDelivery{}))
		form := &legacyForm{ID: 1, PublicID: "legacy-email", Name: "Legacy email", Slug: "legacy-email", Token: "token", AllowedOrigins: "*"}
		require.NoError(t, db.Create(form).Error)
		recipient := "owner@example.com"
		require.NoError(t, db.Create(&partiallySimplifiedEmailDelivery{ID: 1, FormID: form.ID, Enabled: true, Recipient: &recipient}).Error)

		require.NoError(t, Migrate(db))
		require.NoError(t, Migrate(db))
		require.NoError(t, db.Create(&forms.EmailDelivery{
			FormID: form.ID, Name: "Second", RecipientSource: forms.EmailRecipientStatic,
			Recipient: "archive@example.com", ReplyToSource: forms.EmailReplyToNone, Format: forms.EmailFormatText,
		}).Error)

		var deliveries []forms.EmailDelivery
		require.NoError(t, db.Where("form_id = ?", form.ID).Order("id").Find(&deliveries).Error)
		require.Len(t, deliveries, 2)
		require.Equal(t, forms.DefaultEmailDeliveryName, deliveries[0].Name)
		require.Equal(t, forms.EmailRecipientStatic, deliveries[0].RecipientSource)
		require.Equal(t, "owner@example.com", deliveries[0].Recipient)
	})
