...
func TestForms(t *testing.T) {
	...
	t.Run("creates, loads, and rejects duplicate forms", func(t *testing.T) {
		...
		require.NoError(t, err)
		delivery := forms.PrimaryEmailDelivery(loaded)
		require.NotNil(t, delivery)
		require.NotNil(t, loaded.WebhookDelivery)
		assert.True(t, delivery.Enabled)
	...
	t.Run("normalizes email formats and recipient lists", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Equal(t, "PiùUDITO <support@example.com>", forms.DisplayEmailAddresses("=?utf-8?q?Pi=C3=B9UDITO?= <support@example.com>"))
	})
	...
	t.Run("stores nested submissions and schedules enabled deliveries", func(t *testing.T) {
		...
		require.NoError(t, db.Preload("WebhookDelivery").Preload("EmailDeliveries").First(&form, form.ID).Error)
	...
	t.Run("schedules one durable event for each enabled email notification", func(t *testing.T) {
		...
		internal, err := forms.CreateEmailDelivery(logger, db, forms.EmailDeliveryParams{
			FormID: form.ID, Name: "Internal", Enabled: true, MailerProfileID: &profile.ID,
			RecipientSource: forms.EmailRecipientStatic, Recipient: "team@example.com",
			ReplyToSource: forms.EmailReplyToField, ReplyTo: "email",
			Format: forms.EmailFormatHTML, HTMLTemplate: `<p>{{.Fields.message}}</p>`,
		})
		require.NoError(t, err)
		customer, err := forms.CreateEmailDelivery(logger, db, forms.EmailDeliveryParams{
			...
			ReplyToSource: forms.EmailReplyToStatic, ReplyTo: "Support <support@example.com>",
			SubjectTemplate: "We received your request, {{.Fields.name}}", Format: forms.EmailFormatText,
		})
		require.NoError(t, err)
		...
		submission, err := forms.CreateSubmissionWithFiles(logger, db, form, map[string]any{
			"name": "Ada", "email": "ada@example.com", "message": "Call me",
		}, "Browser", "", nil)
		require.NoError(t, err)

		var events []forms.EmailEvent
		require.NoError(t, db.Where("submission_id = ?", submission.ID).Order("email_delivery_id").Find(&events).Error)
		require.Len(t, events, 2)
		assert.Equal(t, []uint{internal.ID, customer.ID}, []uint{*events[0].EmailDeliveryID, *events[1].EmailDeliveryID})

		require.NoError(t, forms.DeleteEmailDelivery(logger, db, form.ID, internal.ID))
		var preserved forms.EmailEvent
		require.NoError(t, db.First(&preserved, events[0].ID).Error)
		assert.Nil(t, preserved.EmailDeliveryID)
	})
	...
	t.Run("scrubs the honeypot and skips files and deliveries", func(t *testing.T) {
		...
		form := forms.Form{
			...
			WebhookDelivery: &forms.WebhookDelivery{Enabled: true, URL: "https://example.com/hook"},
			EmailDeliveries: []forms.EmailDelivery{{Enabled: true, Recipient: "owner@example.com"}},
		}
		require.NoError(t, db.Create(&form).Error)
	...
	t.Run("updates form and delivery settings atomically", func(t *testing.T) {
		...
		delivery := forms.PrimaryEmailDelivery(updated)
		require.NotNil(t, delivery)
		require.NotNil(t, updated.WebhookDelivery)
		assert.True(t, delivery.Enabled)
		assert.Equal(t, "team@example.com, archive@example.com", delivery.Recipient)
		assert.Equal(t, forms.EmailFormatHTML, delivery.Format)
	...
	t.Run("keeps forms without email notifications empty", func(t *testing.T) {
		db := testsupport.SetupTestDB(t)
		form, err := forms.Create(logger, db, forms.CreateParams{
			Name: "No email", Slug: "no-email", AllowedOrigins: "*",
			EmailName: "Primary notification", EmailSubject: "New submission for {{.FormName}}",
			EmailText: "{{range .FieldList}}{{.Name}}: {{.Value}}\n{{end}}",
		})
		require.NoError(t, err)
		assert.Empty(t, form.EmailDeliveries)

		delivery, err := forms.CreateEmailDelivery(logger, db, forms.EmailDeliveryParams{
			FormID: form.ID, Name: "Temporary", Recipient: "owner@example.com",
		})
		require.NoError(t, err)
		require.NoError(t, forms.DeleteEmailDelivery(logger, db, form.ID, delivery.ID))

		updated, err := forms.Update(logger, db, forms.UpdateParams{
			ID: form.ID, Name: "Still no email", AllowedOrigins: "*",
			EmailName: "Primary notification", EmailSubject: "New submission for {{.FormName}}",
			EmailText: "{{range .FieldList}}{{.Name}}: {{.Value}}\n{{end}}",
		})
		require.NoError(t, err)
		assert.Empty(t, updated.EmailDeliveries)
	})
