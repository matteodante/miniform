...
const (
	...
	EmailRecipientStatic    = "static"
	EmailRecipientField     = "field"
	EmailReplyToNone        = "none"
	EmailReplyToStatic      = "static"
	EmailReplyToField       = "field"
)
...
type Form struct {
	...
	EmailDeliveries []EmailDelivery  `gorm:"constraint:OnDelete:CASCADE"`
...
type EmailDelivery struct {
	...
	Name            string                      `gorm:"size:255;not null;default:Email notification"`
	Enabled         bool                        `gorm:"not null;default:false"`
	MailerProfileID *uint                       `gorm:"index"`
	MailerProfile   *integrations.MailerProfile `gorm:"constraint:OnDelete:RESTRICT"`
	RecipientSource string                      `gorm:"size:16;not null;default:static"`
	Recipient       string                      `gorm:"type:text"`
	ReplyToSource   string                      `gorm:"size:16;not null;default:none"`
	ReplyTo         string                      `gorm:"type:text"`
	SubjectTemplate string                      `gorm:"type:text"`
	Format          string                      `gorm:"size:16;not null;default:text"`
	TextTemplate    string                      `gorm:"type:text"`
	HTMLTemplate    string                      `gorm:"type:text"`
	CreatedAt       time.Time
	UpdatedAt       time.Time
}
...
type EmailEvent struct {
	...
	Submission      *Submission    `gorm:"constraint:OnDelete:CASCADE"`
	EmailDeliveryID *uint          `gorm:"index"`
	EmailDelivery   *EmailDelivery `gorm:"constraint:OnDelete:SET NULL"`
	Status          string         `gorm:"size:32;index;not null"`
	AttemptCount    int            `gorm:"not null;default:0"`
	LastAttemptErr  string         `gorm:"type:text"`
...
func NewEmailEvent(submissionID uint, scheduledAt time.Time, emailDeliveryID ...uint) *EmailEvent {
	nextAttempt := scheduledAt.UTC()
	event := &EmailEvent{
		SubmissionID: submissionID, Status: WebhookStatusPending, NextAttemptAt: &nextAttempt,
	}
	if len(emailDeliveryID) > 0 {
		event.EmailDeliveryID = &emailDeliveryID[0]
	}
	return event
}
