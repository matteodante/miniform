...
type storageSnapshot struct {
	...
	webhookDelivered, webhookOutstanding, webhookFailed         int
	emailDelivered, emailOutstanding, emailFailed, emailRetried int
	sequences                                                   map[int]int
	dataBytes, walBytes                                         int64
}

func TestCoreStress(t *testing.T) {
	t.Run("blocks SMTP recipients outside the reserved test domain", func(t *testing.T) {
		capture := newSMTPCapture(t, 0)
		client, err := smtp.Dial(net.JoinHostPort(capture.host, strconv.Itoa(capture.port)))
		require.NoError(t, err)
		t.Cleanup(func() { _ = client.Close() })
		require.NoError(t, client.Mail("guard@sender.invalid"))
		err = client.Rcpt("guard@recipient.example")
		require.Error(t, err)
		assert.Contains(t, err.Error(), "550")

		snapshot := capture.snapshot()
		assert.Empty(t, snapshot.messages)
		assert.Zero(t, snapshot.attempts)
		require.Len(t, snapshot.errors, 1)
		assert.Contains(t, snapshot.errors[0], "blocked non-test recipient")
	})
	...
	t.Run("preserves ingestion delivery storage and lifecycle under load", func(t *testing.T) {
		...
		smtp := newSMTPCapture(t, smtpFailures)
		loadForm, deliveryForm := seedStressForms(t, databasePath, capture.server.URL, smtp.host, smtp.port)
		...
		waitForSMTPCount(t, process, smtp, load.deliveries*2, 45*time.Second)
		...
		expectedEmails := expectedDeliveries * 2
		...
		assert.Equal(t, expectedEmails, firstSnapshot.emailEvents)
		assert.Equal(t, load.deliveries, firstSnapshot.webhookDelivered)
		assert.Equal(t, 1, firstSnapshot.webhookOutstanding)
		assert.Zero(t, firstSnapshot.webhookFailed)
		assert.Equal(t, load.deliveries*2, firstSnapshot.emailDelivered)
		assert.Equal(t, 2, firstSnapshot.emailOutstanding)
		assert.Zero(t, firstSnapshot.emailFailed)
		assert.Equal(t, smtpFailures, firstSnapshot.emailRetried)
		...
		waitForSMTPCount(t, restarted, smtp, expectedEmails, 15*time.Second)
		...
		assert.Equal(t, expectedEmails, finalSnapshot.emailEvents)
		assert.Equal(t, expectedDeliveries, finalSnapshot.webhookDelivered)
		assert.Zero(t, finalSnapshot.webhookOutstanding)
		assert.Zero(t, finalSnapshot.webhookFailed)
		assert.Equal(t, expectedEmails, finalSnapshot.emailDelivered)
		assert.Zero(t, finalSnapshot.emailOutstanding)
		assert.Zero(t, finalSnapshot.emailFailed)
		assert.Equal(t, smtpFailures, finalSnapshot.emailRetried)
		...
		emails := smtp.snapshot()
		assert.Empty(t, emails.errors)
		assert.Equal(t, expectedEmails, len(emails.messages))
		assert.Equal(t, smtpFailures, emails.rejected)
		assert.Equal(t, expectedEmails+smtpFailures, emails.attempts)
		emailSequences := deliverySequences(plans)
		emailSequences = append(emailSequences, interrupted.sequence)
		validateSMTPCapture(t, emails, emailSequences)
		...
		t.Logf("profile: requests=%d valid=%d invalid=%d uploads=%d webhooks=%d emails=%d concurrency=%d throughput=%.1f req/s",
			len(plans), load.valid, load.invalid, load.uploads, expectedDeliveries, expectedEmails, cfg.concurrency, throughput)
...
func seedStressForms(t *testing.T, databasePath, webhookURL, smtpHost string, smtpPort int) (*forms.Form, *forms.Form) {
	t.Helper()
	...
	profile, err := integrations.CreateMailerProfile(logger, db, integrations.MailerProfileParams{
		Name: "Stress SMTP capture", DefaultFromName: "Miniform stress",
		DefaultFromEmail: "no-reply@sender.invalid", SMTPHost: smtpHost,
		SMTPPort: smtpPort, SMTPEncryption: "none",
	})
	require.NoError(t, err)
	...
	deliveryForm, err := forms.Create(logger, db, forms.CreateParams{
		...
		EmailReplyToType:   forms.EmailReplyToField, EmailReplyTo: "email",
		EmailSubject: "Internal {{.Fields.sequence}}", EmailFormat: forms.EmailFormatHTML,
...
func deliverySequences(plans []requestPlan) []int {
	sequences := make([]int, 0)
	for _, plan := range plans {
		if plan.delivery {
			sequences = append(sequences, plan.sequence)
		}
	}
	return sequences
}
...
func buildRequest(ctx context.Context, baseURL string, plan requestPlan) (*http.Request, error) {
	...
	name := fmt.Sprintf("Load %d", plan.sequence)
	email := fmt.Sprintf("customer-%d@recipient.invalid", plan.sequence)
	message := `<script>alert("smtp")</script> & "quotes"`
	values := url.Values{
		"sequence": {strconv.Itoa(plan.sequence)}, "kind": {"stress"},
	...
	case plan.upload:
		...
		for _, field := range []string{"sequence", "kind", "name", "email", "message"} {
			if err := writer.WriteField(field, values.Get(field)); err != nil {
				return nil, err
			}
		}
	...
	case plan.json:
		encoded, err := json.Marshal(map[string]any{
			"sequence": plan.sequence, "kind": "stress", "name": name, "email": email, "message": message,
		})
		if err != nil {
			return nil, err
		}
...
func inspectStorage(t *testing.T, databasePath, dataDir string) storageSnapshot {
	...
	snapshot.webhookDelivered = rowCount(t, db.Model(&forms.WebhookEvent{}).Where("status = ?", forms.WebhookStatusDelivered))
	snapshot.webhookOutstanding = rowCount(t, db.Model(&forms.WebhookEvent{}).Where("status IN ?", []string{
		forms.WebhookStatusPending, forms.WebhookStatusDelivering, forms.WebhookStatusRetrying,
	}))
	snapshot.webhookFailed = rowCount(t, db.Model(&forms.WebhookEvent{}).Where("status = ?", forms.WebhookStatusFailed))
	snapshot.emailDelivered = rowCount(t, db.Model(&forms.EmailEvent{}).Where("status = ?", forms.WebhookStatusDelivered))
	snapshot.emailOutstanding = rowCount(t, db.Model(&forms.EmailEvent{}).Where("status IN ?", []string{
		forms.WebhookStatusPending, forms.WebhookStatusDelivering, forms.WebhookStatusRetrying,
	}))
	snapshot.emailFailed = rowCount(t, db.Model(&forms.EmailEvent{}).Where("status = ?", forms.WebhookStatusFailed))
	snapshot.emailRetried = rowCount(t, db.Model(&forms.EmailEvent{}).Where("attempt_count > 1"))
	require.NoError(t, manager.Close())
